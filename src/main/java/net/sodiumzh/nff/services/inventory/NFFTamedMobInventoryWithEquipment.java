 ...
 */
public class NFFTamedMobInventoryWithEquipment extends NFFTamedMobInventory
{
	public NFFTamedMobInventoryWithEquipment(int size)
	{
		super(size);
		if (size < 6)
			throw new IllegalArgumentException("NFFTamedMobInventoryWithEquipment must have at least 6 slots for a mob's equipment slots.");
	}
	
	public NFFTamedMobInventoryWithEquipment(int size, INFFTamed owner)
	{
		super(size, owner);
	}
