...
public class NFFEntityEventListeners
{
	...
	public static void onLivingSetTarget(LivingSetAttackTargetEvent event)
	{
		...
		{ 	
	        ...
	        // Handle Golems End
	        // Handle tag nffservices:neutral_to_nff_mobs
	        if (mob.getType().is(NFFTagRegistry.NEUTRAL_TO_BM_MOBS) && target instanceof INFFTamed && mob.getLastHurtByMob() != target)
	        	event.setCanceled(true);
		}
		// Handle mobs end //
	}	
	...
	{
		NFFMobRespawnerInstance ins = NFFMobRespawnerInstance.create(event.getEntityItem().getItem());
