...
public interface INFFFollowOwnerGoal
{
	...
	public default void teleportToOwner() {
		...
		for (int i = 0; i < 20; ++i) {
			...
			if (goal().shouldAvoidSun.test(goal().getMob()) && NFULevelStatics.isUnderSun(NFUMathStatics.getBlockPos(pos), mob))
				continue;
	...
	public default boolean tryTeleportToOwner() {
		...
		if (!this.posNoCollision(targetPos))
			return false;
		if (goal().shouldAvoidSun.test(goal().getMob()) && NFULevelStatics.isUnderSun(NFUMathStatics.getBlockPos(targetPos), mob))
			return false;
		else {
