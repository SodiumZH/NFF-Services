	...
	public void serverTick(Mob mob)
	{
		/*mob.getCapability(NFFCapRegistry.CAP_BEFRIENDABLE_MOB).ifPresent((l) -> {
			if (!shouldIgnoreHatred())
			{
				...
				{
					if (l.hasPlayerData(mob.level().getPlayerByUUID(uuid), NBT_KEY_PROGRESS_VALUE)
							&& l.getPlayerDataDouble(mob.level().getPlayerByUUID(uuid), NBT_KEY_PROGRESS_VALUE) > 0d)
