...
public class NFFMobOwnershipTransfererItem extends NaUtilsItem
{
	...
	{
		if (stack == null || stack.isEmpty() || !(stack.getItem() instanceof NFFMobOwnershipTransfererItem))
	...
	{
		if (!isWritten(stack))
			throw new IllegalStateException("NFFMobOwnershipTransfererItem#isLocked: the item isn't written.");
		else return stack.getTag().getBoolean("mot_locked");
	}
	
	public ItemStack fromMob(INFFTamed mob, NFFMobOwnershipTransfererItem itemType)
	{
		ItemStack stack = new ItemStack(itemType);
		if (stack == null || stack.isEmpty() || !(stack.getItem() instanceof NFFMobOwnershipTransfererItem))
			throw new IllegalArgumentException();
		if (!mob.isOwnerPresent())
			throw new IllegalStateException("NFFMobOwnershipTransfererItem writing requires the owner to be present in the level.");
	 ...
	 */
	protected InteractionResult tryTransfer(ItemStack stack, Player player, INFFTamed mob)
	{
		// Wrong stack type
		if (stack == null || stack.isEmpty() || !(stack.getItem() instanceof NFFMobOwnershipTransfererItem))
			throw new IllegalArgumentException();
